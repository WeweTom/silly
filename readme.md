...
- 组合任务
  ...
  通过创建tasks.json文件(作用类似于GruntFile，但是更加简单直接)，来配置执行组合任务
...
```
  silly默认会寻找当前目录下的tasks.json，所以上面的命令可以简化为
...
```shell
$ silly compile your_page.less # 将your_page.less编译为your_page.css
```
...
$ cd kissypie			# tasks.json 配置完后执行下面命令
$ silly run
```
#### SinglPage
